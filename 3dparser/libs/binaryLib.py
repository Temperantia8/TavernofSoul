...
class BinaryReader:
    ...
    def logWrite(self,data):
        if self.logfile is not None:
            ...
            logging.warning( 'WARNING: no log')
    ...
    def findchar(self,var):
        ...
        if self.debug==True:
            logging.debug (var,'znaleziono',offset)
        if self.log==True:
            if self.logfile is not None:
    ...
    def tell(self):
        ...
        if self.debug==True:
            logging.debug ( 'current offset is',val)
        return val  
    ...
    def word(self,long):
        if long<10000:
            ...
            if self.debug==True:
                logging.debug ( 'WARNING:too long')
            #return 1
