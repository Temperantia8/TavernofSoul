...
def getclass_fromstring(c, string):
    ...
    bonus = string.lower().replace('{nl}',' ').replace('\xa0', ' ').split(' ')
    ...
    for i in range(len(bonus)):
        ...
        elif ' '.join(bonus[i:i+2]) in job_names:
             rank[bow[' '.join(bonus[i:i+2])]] +=5
             found.append(bonus[i:1+2])
             
             
        elif bonus[i] in bow_key:
            rank[bow[bonus[i]]] +=1
            found.append(bonus[i])
        elif ' '.join(bonus[i:i+2]) in bow_key:
            rank[bow[' '.join(bonus[i:i+2])]] +=3
            found.append(bonus[i:i+2])
        elif  ' '.join(bonus[i:i+3]) in bow_key:
